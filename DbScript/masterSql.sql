...
CREATE TABLE `candidate` (
  ...
  KEY `batchIdCandidate_idx` (`batchId`),
  CONSTRAINT `batchIdCandidate` FOREIGN KEY (`batchId`) REFERENCES `batchdetails` (`batchId`) ON DELETE NO ACTION ON UPDATE NO ACTION
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
/*!40101 SET character_set_client = @saved_cs_client */;
...
CREATE TABLE `uploadeddocuments` (
  ...
  KEY `batchIdUploadedDocument_idx` (`batchId`),
  CONSTRAINT `batchIdUploadedDocument` FOREIGN KEY (`batchId`) REFERENCES `batchdetails` (`batchId`) ON DELETE NO ACTION ON UPDATE NO ACTION,
...
-- Dump completed on 2018-07-19 12:13:36
